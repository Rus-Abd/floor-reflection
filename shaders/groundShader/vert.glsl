...
float average(const in vec3 v){return dot(v,vec3(.3333333));}
...
float luminance(const in vec3 rgb){
    ...
    const vec3 weights=vec3(.2126729,.7151522,.0721750);
...
vec2 equirectUv(in vec3 dir){
    ...
    float u=atan(dir.z,dir.x)*RECIPROCAL_PI2+.5;
    
    float v=asin(clamp(dir.y,-1.,1.))*RECIPROCAL_PI+.5;
...
void main(){
    ...
    if(isPerspectiveMatrix(projectionMatrix)){
        ...
        gl_Position.z=log2(max(EPSILON,gl_Position.w+1.))*logDepthBufFC-1.;
